...
from models import Candidate, TestDecisionLog
...
def candidate_key(norm_mobile: str, candidate_name: Any) -> str:
    return f"{str(norm_mobile or '')}|{normalize_candidate_name(candidate_name)}"


def has_duplicate_candidate_in_requirement(db, *, requirement_id: str, norm_mobile: str, candidate_name: str) -> bool:
    key = candidate_key(norm_mobile, candidate_name)
    if not key or key == "|":
        return False
    rows = (
        db.execute(select(Candidate.candidateName, Candidate.mobile).where(Candidate.requirementId == requirement_id))
        .all()
    )
