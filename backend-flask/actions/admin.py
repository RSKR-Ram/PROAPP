...
def users_list(data, auth: AuthContext | None, db, cfg):
    ...
    for u in rows:
        ...
        if status_uc and str(item["status"]).upper() != status_uc:
            continue
        if q_lc:
            hay = f"{item['email']} {item['fullName']} {item['role']}".lower()
            if q_lc not in hay:
                continue
        items.append(item)

    items.sort(key=lambda x: str(x.get("email", "")))
    total = len(items)
...
def users_upsert(data, auth: AuthContext | None, db, cfg):
    ...
    now = iso_utc_now()
    updated_by = auth.email if auth else ""
...
def settings_get(data, auth: AuthContext | None, db, cfg):
    ...
    if not rows:
        return {"items": [defaults["PASSING_MARKS"], defaults["NOT_PICK_THRESHOLD"]]}
    ...
    out = [mp.get("PASSING_MARKS", defaults["PASSING_MARKS"]), mp.get("NOT_PICK_THRESHOLD", defaults["NOT_PICK_THRESHOLD"]), mp.get("INTERVIEW_MESSAGE_TEMPLATE", defaults["INTERVIEW_MESSAGE_TEMPLATE"])]
    return {"items": out}
